...
const Item = ({ size, day, isCheck }: ItemPropsType) => {
  return (
    <View
      style={[
        styles.recordBox,
        styles[isCheck ? 'enabled' : 'disabled'],
        {
          width: size,
          height: size,
        },
...
const GoalCalendar = () => {
  ...
  const DataChecking = (data?: string[]) => {
    ...
    const searchDataFn = (arr: Array<string>, date: Date) =>
      arr.indexOf(`${date.getFullYear()}-${twoWord(date.getMonth() + 1)}-${twoWord(date.getDate())}`);
    const firstDate = sortedData.at(-1) ? new Date(sortedData.at(-1) as string) : new Date();
    let dataSliceTmp = 0;
    const length =
      (nowDate.getFullYear() - firstDate.getFullYear()) * 12 + (nowDate.getMonth() - firstDate.getMonth()) + 1;
